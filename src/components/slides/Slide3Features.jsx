...
const containerVariants = {
  animate: { transition: { staggerChildren: 0.4, delayChildren: 0.4 } },
}
      ...
      >
        {features.map(({ icon: Icon, color, bg, title, detail, extra }) => (
          <motion.div
            ...
            className="flex-1 flex flex-col items-center gap-3 rounded-2xl p-8 shadow-md border border-gray-100 bg-white"
          >
            <motion.div
            ...
            {extra && <p className="text-xl">{extra}</p>}
          </motion.div>
        ))}
      </motion.div>
    </div>
